...
namespace SEFL
{
	...
	LEDMatrixPuzzleNeopixel::~LEDMatrixPuzzleNeopixel()
	{
		// TODO Auto-generated destructor stub
	}

	void LEDMatrixPuzzleNeopixel::onDefault()
	{
	...
	void LEDMatrixPuzzleNeopixel::onActive()
	{
		if (this->changed_status_)
		{
			this->changed_status_ = false;
	...
	void LEDMatrixPuzzleNeopixel::onFinished()
	{
		if (this->changed_status_)
		{
			this->changed_status_ = false;
	...
	void LEDMatrixPuzzleNeopixel::onManualFinished()
	{
		if (this->changed_status_)
		{
			this->changed_status_ = false;
	...
	LEDMatrixPuzzleNeopixel::LEDMatrixPuzzleNeopixel(MQTTClient &mqtt, const char *name,
	...
	void LEDMatrixPuzzleNeopixel::initButtons()
	{
	...
	void LEDMatrixPuzzleNeopixel::scanButtons()
	{
	...
	void LEDMatrixPuzzleNeopixel::refreshStrip()
	{
		Mext.digitalRead(this->strip_pin);
        pinMode(Mext.getCi(),INPUT);
		strip.begin();
	...
	bool LEDMatrixPuzzleNeopixel::checkPattern()
	{
	...
	void LEDMatrixPuzzleNeopixel::setStripPin(uint8_t pin)
	{
		if (pin >= 0 && pin < 16)
		{
			this->strip_pin = pin;
	...
	void LEDMatrixPuzzleNeopixel::setPattern(const uint8_t *array, uint8_t size)
	{
	...
	void LEDMatrixPuzzleNeopixel::setButtons(const uint8_t *array, uint8_t size)
	{
	...
	uint8_t *LEDMatrixPuzzleNeopixel::getButtons()
	{
		return this->buttons_pins;
	}
	uint8_t LEDMatrixPuzzleNeopixel::getButtonsSize()
	{
		return this->buttons_amount;
	}
	void LEDMatrixPuzzleNeopixel::setStripSegmentSize(uint8_t segment_size)
	{
		if (segment_size != 0)
		{
	...
	uint32_t LEDMatrixPuzzleNeopixel::getColor(uint8_t color)
	{
