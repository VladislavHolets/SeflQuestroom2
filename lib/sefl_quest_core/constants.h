...
static const char *const CLIENT_STRINGS[] MEM_TYPE = { DEFAULT_DEVICE_TYPE,
		ACTUATOR_DEVICE_TYPE, BASIC_DEVICE_TYPE, SENSOR_DEVICE_TYPE,
		START_BUTTON_DEVICE_TYPE, TIMER_DEVICE_TYPE, MEDIA_PLAYER_DEVICE_TYPE,
		QUEST_CONTROL_SERVER_DEVICE_TYPE, HOST_DEVICE_TYPE, };
...
enum ActuatorClientStatuses {
	OFF_STATUS = 1, ON_STATUS = 2
};

enum SensorClientStatuses {
	SET_STATUS = 1, RESET_STATUS = 2
};
...
enum TimerClientStatuses {
	FULL_STATUS = 1, ENABLED_STATUS = 2, FREE_STATUS = 3, TRIGGERED_STATUS = 4
};
...
enum BasicClientActionSubcommands {
	ACTIVATE = 1, DEACTIVATE = 2, SOLVE = 3
};
enum Language {
	ENG = 2, UKR = 1, RUS = 0
};
